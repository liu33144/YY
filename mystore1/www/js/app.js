...
.controller("lctrl",function($scope){
		$scope.bt=function(){
			...
			function Log(data){
				if(data.login){
                    ...
                    limg.src = "img/"+data.img+".jpg";
                    lname.innerHTML = data.uname;
				}else {
					console.log(data.msg);
    ...
    .controller('lctrl1',function($scope,$http){
        $http({
            url:'js/img.json'
        }).success(function(data){
            console.log(data);
        ...
        $scope.tuu = function(){
            console.log(this.ee.img);
            xx = this.ee.img.indexOf("/");
            xx = this.ee.img.slice(xx+1,xx+2);
            console.log(xx);
            //this.ee.style.borderRadius = "1px solid red";
        ...
        $scope.btt=function(){
            ...
            if(xx == ""){
                var imgE = document.getElementsByClassName("img-err");
                imgE[0].innerHTML = "请选择头像！";
                imgE[0].style.color = "red";
                return;
            }
             var good = "pwd.php";
             getJson(good,uname,pwd,xx,Loo);
	...
	.controller('lctrl2',function($scope){
            ...
            var pwd = document.getElementById("pwd").value;
            var good = "users.php";
            getJson(good,uname,pwd,"",Lgg);
            function Lgg(data){
                $scope.limg = data.img;
                $scope.lname = data.uname;
            }
	})

function getJson(address,uname,pwd,img,Good){
	var http;
	...
	http.send("uname="+ uname + "&pwd="+pwd+"&img="+img);
}
